...
static rtk_api_ret_t _rtk_rate_stormControlPortEnable_get(rtk_port_t port, rtk_rate_storm_group_t stormType, rtk_enable_t *pEnable)
{
    ...
    {
    case STORM_GROUP_UNKNOWN_UNICAST:
        if ((retVal = rtl8367c_getAsicStormFilterUnknownUnicastEnable(rtk_switch_port_L2P_get(port), (rtk_uint32*)pEnable)) != RT_ERR_OK)
            return retVal;
        break;
    case STORM_GROUP_UNKNOWN_MULTICAST:
        if ((retVal = rtl8367c_getAsicStormFilterUnknownMulticastEnable(rtk_switch_port_L2P_get(port), (rtk_uint32*)pEnable)) != RT_ERR_OK)
            return retVal;
        break;
    case STORM_GROUP_MULTICAST:
        if ((retVal = rtl8367c_getAsicStormFilterMulticastEnable(rtk_switch_port_L2P_get(port), (rtk_uint32*)pEnable)) != RT_ERR_OK)
            return retVal;
        break;
    case STORM_GROUP_BROADCAST:
        if ((retVal = rtl8367c_getAsicStormFilterBroadcastEnable(rtk_switch_port_L2P_get(port), (rtk_uint32*)pEnable)) != RT_ERR_OK)
            return retVal;
        break;
    default:
        break;
...
static rtk_api_ret_t _rtk_storm_bypass_get(rtk_storm_bypass_t type, rtk_enable_t *pEnable)
{
    ...
    {
        if ((retVal = rtl8367c_getAsicIGMPBypassStormCTRL((rtk_uint32*)pEnable)) != RT_ERR_OK)
            return retVal;
    }
    else if (type == BYPASS_CDP)
    {
