...
static rtk_api_ret_t _rtk_oam_state_get(rtk_enable_t *pEnabled)
{
    ...
    if ((retVal = rtl8367c_getAsicOamEnable((rtk_uint32*)pEnabled)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_oam_parserAction_get(rtk_port_t port, rtk_oam_parser_act_t *pAction)
{
    ...
    if ((retVal = rtl8367c_getAsicOamParser(rtk_switch_port_L2P_get(port), (rtk_uint32*)pAction)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_oam_multiplexerAction_get(rtk_port_t port, rtk_oam_multiplexer_act_t *pAction)
{
    ...
    if ((retVal = rtl8367c_getAsicOamMultiplexer(rtk_switch_port_L2P_get(port), (rtk_uint32*)pAction)) != RT_ERR_OK)
        return retVal;
