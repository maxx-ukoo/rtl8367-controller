...
static rtk_api_ret_t _rtk_qos_1pRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtl8367c_getAsicRemarkingDot1pAbility(rtk_switch_port_L2P_get(port), (rtk_uint32*)pEnable)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_qos_1pRemarkSrcSel_get(rtk_qos_1pRmkSrc_t *pType)
{
    ...
    if ((retVal = rtl8367c_getAsicRemarkingDot1pSrc((rtk_uint32*)pType)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_qos_dscpRemarkEnable_get(rtk_port_t port, rtk_enable_t *pEnable)
{
    ...
    if ((retVal = rtl8367c_getAsicRemarkingDscpAbility((rtk_uint32*)pEnable)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_qos_dscpRemarkSrcSel_get(rtk_qos_dscpRmkSrc_t *pType)
{
    ...
    if ((retVal = rtl8367c_getAsicRemarkingDscpSrc((rtk_uint32*)pType)) != RT_ERR_OK)
        return retVal;
...
static rtk_api_ret_t _rtk_qos_portPriSelIndex_get(rtk_port_t port, rtk_qos_priDecTbl_t *pIndex)
{
    ...
    if ((retVal = rtl8367c_getAsicPortPriorityDecisionIndex(rtk_switch_port_L2P_get(port), (rtk_uint32*)pIndex)) != RT_ERR_OK)
        return retVal;
